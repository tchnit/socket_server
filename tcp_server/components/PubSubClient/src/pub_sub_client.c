...
void data_to_mqtt(char *data, char *topic, int delay_time_ms, int qos)
{
    ...
    if (len > 0) 
    {
        ...
        if(esp_mqtt_client_publish(g_mqtt_client, topic, json_data, strlen(json_data), qos, 0) != -1)
        {
            printf("Message sent: %s \n", json_data);
        }
        else if (esp_mqtt_client_publish(g_mqtt_client, topic, json_data, strlen(json_data), qos, 0) == -1)
        {
            ESP_LOGE(MQTT_TAG, "Failed to publish data!");
        }
        else if (esp_mqtt_client_publish(g_mqtt_client, topic, json_data, strlen(json_data), qos, 0) == -2)
        {
